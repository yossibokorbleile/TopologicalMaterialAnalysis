	...
	for config in config_files:
		...
		for s in sample_every:
			...
			if save_all:
				...
				fig = plot_APF(APF_1, config_name)
				...
				fig = plot_APF(APF_2, config_name)
...
def batch_mode_kernel(parent_dir : str, file_ext : str, format : str, structure_file : str, structure : str, sample_range : str, kernel_settings : str, save_all : bool = False):
	...
	for config in config_files:
		...
		for s in sample_every:
			...
			params = read_oineus_settings(structure_file, kernel_settings)
			...
			if params.kernel:
				...
				try:
					...
					fig = plot_PD(pd_1[:,0], pd_1[:,1], config_name)
				...
				try:
					...
					fig = plot_PD(pd_2[:,0], pd_2[:,1], config_name)
