...
class Reeb_Graph(object):
    ...
    def __init__(
        ...
        if self.grid_size % self.reeb_stride == 0:
            self.grid_size += 1

        if self.verbose and self.periodic:
            print(
                "This number should be ONE for accurate periodic graphs!",
                self.grid_size % self.reeb_stride,
            )
            print(
                "Coverings should be symmetric for accurate periodic graphs!",
                np.min(self.covering),
                np.max(self.covering),
            )
            print(
                "Coverings and stride should be coherent: stride = covering + 1 ! Stride: ",
                self.reeb_stride,
                "Covering Step: ",
                np.max(self.covering),
            )
        self.setup_grid(
            balls_centres=self.backbone, balls_radii=self.radii, M=self.M, m=self.m
        )
    ...
    def setup_grid(self, balls_centres=None, balls_radii=None, M=None, m=None):
        if self.transform_points is not None:
            balls_centres, balls_radii = self.transform_backbone(
                balls_centres=balls_centres, balls_radii=balls_radii, M=M, m=m
            )

        self.produce_res_grid(
            balls_centres=balls_centres, balls_radii=balls_radii, M=M, m=m
        )

    def transform_backbone(self, balls_centres=None, balls_radii=None, M=None, m=None):

        if isinstance(self.transform_points, np.ndarray):
            V, v = self.transform_points, 0
        else:
        ...
        if self.inputfile is not None:
            ...
            balls_centres, balls_radii = self.affine_transform(
                balls_centres, balls_radii, V, v, M, m
            )
    ...
    def produce_res_grid(self, balls_centres=None, balls_radii=None, M=None, m=None):

        if self.verbose:
            print("The filtration radii are: ", balls_radii[0], balls_radii[-1])
        ...
        self.r_graph = (
            np.sqrt(self.d_x**2 + self.d_y**2 + self.d_z**2) * 1.001 * self.fat_radius
        )

        if self.relax_z_axis is not None:
        ...
        if self.distances_to_balls_aux is None:
            ...
            self.distances_to_balls = np.full(len(grid_aux), np.inf)
            self.atoms_kinds = np.full(len(grid_aux), -1, dtype=int)
            ...
            if self.save_RAM:
                ...
                if self.MP:
                    ...
                    for i, d in enumerate(D):
                        self.distances_to_balls = np.vstack(
                            [self.distances_to_balls, d]
                        )
                        r_aux = np.argmin(self.distances_to_balls, axis=0).astype(int)
                        self.distances_to_balls = np.min(
                            self.distances_to_balls, axis=0
                        )

                        self.atoms_kinds[r_aux > 0] = (
                            r_aux[r_aux > 0] - 1 + idxs_grid[i]
                        )
                    ...
                    for i in idxs_grid:
                        ...
                        improved = min_d < self.distances_to_balls
                        self.distances_to_balls[improved] = min_d[improved]
                        self.atoms_kinds[improved] = i + min_idx[improved]

            else:
                D = dist_from_pts_periodic_boundaries_numba(
                    balls_centres, grid_aux, self.M, self.m, axes_aux
        ...
        if self.save_RAM:
            ...
            grid_to_cubic = np.unravel_index(
                np.arange(len(self.grid)), (self.nx, self.ny, self.nz)
            )
            ...
            cubic_to_grid = np.ravel_multi_index(
                (xv, yv, zv), (self.nx, self.ny, self.nz)
            )

            if self.verbose:
                print("Building Graph with Numba.")

            A, cnt = make_graph_fast(
                self.grid,
                self.idxs,
                res_to_grid,
                grid_to_res,
            ...
            self.graph = csr_matrix(
                (np.ones_like(A[:, 0]), (A[:, 0], A[:, 1])), shape=(N, N)
            )

        else:
            D = dist_periodic_boundaries(
                self.res_grid, self.M, self.m, self.axes, self.dim
            )
    ...
    def reeb_graph(self, grid, axis=-1, D_=None):
        ...
        if D_ is None:
            D_graph = safe_dist_from_pts_periodic_boundaries_numba(
                grid, grid, self.M, self.m, self.axes, self.dim
            )
        ...
        self.norm = (
            (self.delta * np.max(self.covering)) // (self.d_z * self.reeb_stride)
        ) + 1
        ...
        reeb_idxs = np.concatenate(
            [reeb_idxs, np.arange(reeb_idxs[-1] + 1, values.shape[0], 1)]
        )
        ...
        lvl_to_grid = np.where(fat_lvl)[0]
        sub = D_graph[np.ix_(lvl_to_grid, lvl_to_grid)]  # single extraction
        n_c, C = connected_components(sub)
        ...
        for j, v in enumerate(reeb_values[1:]):
            ...
            self.REEB_GRAPH[:, i], AUX_w, AUX_f, AUX_e, count = connect_lvl_sets_aux(
                i, n_c, C, grid, lvl_to_grid, self.REEB_GRAPH, self.norm, count
            )
        ...
        weights = csr_matrix(
            (weights[0:, -1], (weights[0:, 0].astype(int), weights[0:, 1].astype(int))),
            shape=(len(fun), len(fun)),
        )
    ...
    def build_graph(self, E, weights, fun, emb):
        ...
        D_0 = (
            shortest_path(
                self.D_reeb_w, directed=True, indices=[0], return_predecessors=False
            )[0]
            < np.inf
        )
        D_1 = (
            shortest_path(
                self.D_reeb_w.T, directed=True, indices=[N], return_predecessors=False
            )[0]
            < np.inf
        )
    ...
    def compute_tunnels(self, n=10, D_=None, independent=False, plot=False):
        ...
        for i in range(n):
            ...
            d, pred = bellman_ford(
                graph, directed=True, indices=[0], return_predecessors=True
            )
    ...
    def network_plot_3D(self, fun, D=None):
        ...
        for i, j in enumerate(self.G.edges()):
            if D is not None and D[j[0], j[1]] == 0:
                c = "r"
            else:
                c = "k"
    ...
    def plot_res_grid(self, three_d=False, heights_=None, axis=-1):
        ...
        if three_d:
            ...
            for h in heights:
                ...
                ax1.scatter(
                    self.res_grid[tmp][:, (axis + 1) % 3],
                    self.res_grid[tmp][:, (axis + 2) % 3],
                    c="b",
                )

    def plot_path_connected_comp(
        self,
        comps,
        three_d=False,
        boundaries=False,
        ...
        if not three_d:
            ...
            if plot_sorrounding:
                _, P, S = self.compute_sorrounding_structure(
                    comps, boundaries=boundaries
                )
    ...
    def compute_sorrounding_structure(self, path, boundaries=False):
        ...
        D = dist_from_pts_periodic_boundaries(
            self.res_grid[comp], self.balls_centres, self.M, self.m, axes_aux
        )
